...
std::vector<int64_t> GPT2Tokenizer::encode(const std::string& text, const int max_len) {
...
std::vector<std::string> GPT2Tokenizer::tokenize(const std::string& text, const int max_len) {
	...
	int current_len = 0;  // Track the current total length of tokens
	for (auto match: ctre::range<pattern>(text)) {
		...
		// Check if adding the current token would exceed max_len
		result.reserve(result.size()+bpe_result.size());
		result.insert(result.end(), bpe_result.begin(), bpe_result.end());
		++current_len;

		if (current_len == max_len) {
			// If adding the token would exceed max_len, break the loop
			break;
		}
	}
